...
import { ShellError, run as runShell } from "./shell.js"
import {
  WindowSampler,
  WindowSamplerError,
  layerStub,
} from "./window-sampler.js"
...
const parseSample = (
  ...
  if (!app || app.length === 0) {
    return Effect.fail(
      new WindowSamplerError({ message: `${source} returned an empty app name` }),
    )
  }
...
const runSample = (
  ...
  runShell(undefined, args).pipe(
    Effect.mapError((error: ShellError): WindowSamplerError =>
      new WindowSamplerError({ message: `${source}: ${error.message}` }),
    ),
...
const makeSampler = (
  ...
  Effect.flatMap(Effect.sync(() => new Date()), (sampledAt) =>
    runSample(source, args, sampledAt),
  )
  ...
  (sampledAt) =>
    Effect.gen(function* () {
      const id = yield* runShell(undefined, ["xdotool", "getactivewindow"]).pipe(
        Effect.mapError(
          (error: ShellError): WindowSamplerError =>
            new WindowSamplerError({ message: `xdotool: ${error.message}` }),
        ),
      )
      const title = yield* runShell(undefined, [
        "xdotool",
        "getwindowname",
        id.trim(),
      ]).pipe(
      ...
      const windowClass = yield* runShell(undefined, [
        "xdotool",
        "getwindowclassname",
        id.trim(),
      ]).pipe(
...
export const linuxLayer = Layer.succeed(
  WindowSampler,
  WindowSampler.of({ sample: xdotoolSample }),
)
