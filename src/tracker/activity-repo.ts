...
export class PersistenceError extends Schema.TaggedErrorClass<PersistenceError>()("ActivityRepo.PersistenceError", {
  message: Schema.String,
}) {}

export interface Interface {
  readonly insertEvent: (event: NewEvent) => Effect.Effect<{ id: number }, SqlError.SqlError | PersistenceError>
  readonly updateEventEnd: (id: number, endAt: Date) => Effect.Effect<void, SqlError.SqlError | PersistenceError>
  readonly listDay: (
    start: Date,
    end: Date,
...
export class ActivityRepo extends Context.Service<ActivityRepo, Interface>()("@app/ActivityRepo") {}

const decodeRows = (rows: ReadonlyArray<unknown>): Effect.Effect<ReadonlyArray<EventRow>, PersistenceError> =>
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const insertEvent = Effect.fn("ActivityRepo.insertEvent")(function* (event: NewEvent) {
    ...
    const updateEventEnd = Effect.fn("ActivityRepo.updateEventEnd")(function* (id: number, endAt: Date) {
      yield* sql`
        UPDATE activity.event SET end_at = ${endAt} WHERE id = ${id}
      `
    })

    const listDay = Effect.fn("ActivityRepo.listDay")(function* (start: Date, end: Date) {
